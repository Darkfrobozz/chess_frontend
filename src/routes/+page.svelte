<script lang="ts">
	...
	import { board_store, piece_store, stats_store } from '$lib/stores/chess_stores';
	...
	var regex = /\/([^\/]+)\.png/;
	$: board = $board_store;
	$: previous_move = get_name($stats_store.last_moved);
	...
	function register_square_click(box: Square): void {
		...
		if (possible_moves.some((pos) => box.id.x == pos.x && box.id.y == pos.y)) {
			...
			let move_to_server = $stats_store.last_move + ';';
		}
...
{:else}
	<div class="flex_with_statitics">
		...
		<div>
			<ul>
				<li>a8 rook moved: {$stats_store.move_tracker.black_rook_a8}</li>
				<li>h8 rook moved: {$stats_store.move_tracker.black_rook_h8}</li>
				<li>h1 rook moved: {$stats_store.move_tracker.white_rook_h1}</li>
				<li>a1 rook moved: {$stats_store.move_tracker.white_rook_a1}</li>
				<li>White king moved: {$stats_store.move_tracker.white_king}</li>
				<li>Black king moved: {$stats_store.move_tracker.black_king}</li>
				<li>Number of moves: {$stats_store.moves}</li>
				<li>Previously moved: {previous_move}</li>
				<li>Previous move: {$stats_store.last_move}</li>
			</ul>
		</div>
		<div class="promotion">
